    ...
    for file in input_files:
        ...
        for v_category in vf["VAGs_category"].unique():
            if v_category in df_filtered["VAGs_category"].values:
                categ_summary.loc[os.path.splitext(file)[0], v_category] = 1
            else:
                categ_summary.loc[os.path.splitext(file)[0], v_category] = 0

        # update genes_summary based on filtered results
        for v_gene in vf["Gene"].unique():
            if v_gene in df_filtered["gene"].values:
                gene_summary.loc[os.path.splitext(file)[0], v_gene] = 1
            else:
                gene_summary.loc[os.path.splitext(file)[0], v_gene] = 0

        # update genes_summary based on filtered results
        for v_locustag in vf["Locus Tag"].unique():
            if v_locustag in df_filtered["qseqid"].values:
                locustag_summary.loc[os.path.splitext(file)[0], v_locustag] = 1
            else:
                locustag_summary.loc[os.path.splitext(file)[0], v_locustag] = 0
